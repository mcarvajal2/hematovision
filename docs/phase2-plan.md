...
- **ML (`ml/`):** `pyproject.toml` declara un entorno **moderno** (TensorFlow ≥2.16,<2.20; Keras ≥3.0,<4.0) — distinto del entorno histórico del `.h5` publicado (Keras 2.10.0, confirmado por HDF5, ver `model-card.md` y HIST-MODELS-004). `ml/src/hematovision_ml/` ya contiene el scaffolding mínimo de Fase 2 (`classes.py`, `model.py`, `freeze.py`, `dataset.py`, `preprocessing.py`, `augmentation.py`, `seeds.py`, `config.py`) y `ml/tests/` tiene pruebas automatizadas que pasan (ver Sección 2) — sin entrenamiento todavía. No hay DVC inicializado (no existe `.dvc/`). El único artefacto de entrenamiento reproducible es el notebook histórico, preservado como evidencia, no como punto de partida ejecutable (ver `AGENTS.md`).
