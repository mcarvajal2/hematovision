...
- **ML (`ml/`):** `pyproject.toml` declara un entorno **moderno** (TensorFlow ≥2.16,<2.20; Keras ≥3.0,<4.0) — distinto del entorno histórico del `.h5` publicado (Keras 2.10.0, confirmado por HDF5, ver `model-card.md` y HIST-MODELS-004). `ml/src/hematovision_ml/` ya contiene el scaffolding mínimo de Fase 2 (`classes.py`, `model.py`, `freeze.py`, `dataset.py`, `preprocessing.py`, `augmentation.py`, `seeds.py`, `config.py`) y `ml/tests/` tiene pruebas automatizadas que pasan (ver Sección 2) — sin entrenamiento todavía. DVC está inicializado con un remoto GCS real conectado y probado (`gs://hematovision-ml-dvc`, ver [DVC y remoto GCS](dvc-plan.md)); los manifiestos y los datasets originales todavía no están versionados ni subidos ahí. El único artefacto de entrenamiento reproducible es el notebook histórico, preservado como evidencia, no como punto de partida ejecutable (ver `AGENTS.md`).
...
**Estado de ejecución (2026-09-08):** tareas 1-5 de la Sección 7 ejecutadas y verificadas de forma independiente — inventario completo en [Manifiesto de originales](dataset-manifest.md) y scaffolding reproducible bajo `ml/src`/`ml/tests`, sin entrenamiento. El manifiesto y el split 80/10/10 con semilla `20260907` quedaron **congelados** el 2026-09-07 (DEC-003), manteniendo la cuarentena DEC-002. Tarea 6 (DVC/remoto) **parcialmente ejecutada**: proyecto GCP, bucket y remoto DVC reales creados y probados (ver [DVC y remoto GCS](dvc-plan.md)), pero el manifiesto y los datasets originales todavía no están versionados/subidos ahí. EXP-REPRO sigue sin autorizar.
...
| 6 | Inicializar DVC y configurar un remoto compartido (GCS), versionar el manifiesto y la asignación de split — **requisito explícito de `ml/README.md` antes de una nueva ronda de entrenamiento**, no opcional. **Infraestructura ya creada y probada** (proyecto `hematovision-ml`, bucket `gs://hematovision-ml-dvc`, remoto `gcs` conectado, prueba push/pull exitosa — ver [DVC y remoto GCS](dvc-plan.md)); **versionar el manifiesto y subir los datasets originales sigue pendiente**, requiere autorización separada | 4, 5 | Configuración de infraestructura (cuenta/credenciales de almacenamiento remoto) — ya resuelta; falta la subida de datos | **Sí, para subir datos reales** — la infraestructura ya está autorizada y creada |
...
**Qué está bloqueado en autorización de cómputo/infraestructura:** 2 (construir el manifiesto, ya ejecutada), 6 (subir manifiesto y datasets reales a GCS — la infraestructura del remoto ya está lista), 7 (EXP-REPRO) y 15 (EXP-COMPARE).
